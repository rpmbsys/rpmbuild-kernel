...
%endif
%if 0%{?rhel} == 7
Requires: fileutils, module-init-tools >= 3.16-2, initscripts >= 8.11.1-1, grubby >= 8.28-2
%endif
...
Source0: https://www.kernel.org/pub/linux/kernel/v5.x/linux-%{tarfile_release}.tar.xz
...
%else
%global perf_make \
  %{__make} %{?make_opts} EXTRA_CFLAGS="${RPM_OPT_FLAGS}" LDFLAGS="%{__global_ldflags} -Wl,-E" %{?cross_opts} -C tools/perf V=1 NO_PERF_READ_VDSO32=1 NO_PERF_READ_VDSOX32=1 WERROR=0 NO_LIBUNWIND=1 HAVE_CPLUS_DEMANGLE=1 NO_GTK2=1 NO_STRLCPY=1 NO_BIONIC=1 %{?perf_build_extra_opts} prefix=%{_prefix} PYTHON=%{__python3}
%endif
%if %{with_perf}
# perf
...
%{expand:%%posttrans %{?1}}\
%if 0%{?rhel} == 7\
%{_sbindir}/new-kernel-pkg --package kernel%{?1:-%{1}} --mkinitrd --dracut --depmod --update %{KVERREL}%{?1:+%{1}} \
rc=$?\
if [ $rc != 0 ]; then\
    %{_sbindir}/new-kernel-pkg --remove %{KVERREL}%{?1:+%{1}}\
    ERROR_MSG="ERROR: installing kernel-%{KVERREL}%{?1:+%{1}}: no space left for creating initramfs. Clean up /boot partition and re-run '%{_sbindir}/new-kernel-pkg --package kernel%{?1:-%{1}} --mkinitrd --dracut --depmod --install %{KVERREL}%{?1:+%{1}}'"\
